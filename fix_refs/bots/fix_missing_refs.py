...
def find_mdwiki_revid(sourcetitle: str) -> str:
    ...
    try:
        ...
        logger.info(f"url{json_file}")
        logger.info(f"count of data: {len(data)}")
...
def get_full_text(sourcetitle: str, mdwiki_revid: int) -> str:
    ...
    if mdwiki_revid == 0:
        logger.info(f"empty mdwiki_revid, sourcetitle:({sourcetitle})")
        return ""
...
def fix_missing_refs(text: str, sourcetitle: str, mdwiki_revid: int = 0) -> str:
    ...
    logger.debug(f"empty refs: {len(short_refs)}")
